...
The ratio between the number of times users clicked on a result and the number of times Meilisearch showed that result. Since users will click results that potentially match what they were looking for, a higher number indicates better relevancy.
