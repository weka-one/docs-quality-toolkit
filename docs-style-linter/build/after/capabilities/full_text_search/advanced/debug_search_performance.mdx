...
<Note>
Multiple occurrences of the same stage (for example, multiple `search > keyword ranking` in a federated query) are automatically accumulated into a single total duration.
</Note>
