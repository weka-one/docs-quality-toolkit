...
Indexing is the process of adding documents to Meilisearch so they become searchable. All indexing operations are [asynchronous](/capabilities/indexing/tasks_and_batches/async_operations), meaning they are added to a task queue and processed in order.
...
Foreign keys let you link documents across indexes. Instead of duplicating data, you store related information in a separate index and reference it by ID. At search time, Meilisearch automatically hydrates results with the full referenced documents. See [Link indexes](/capabilities/indexing/how_to/document_relations) for a step-by-step guide.
