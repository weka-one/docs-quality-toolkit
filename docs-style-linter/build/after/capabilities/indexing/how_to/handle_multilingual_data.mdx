...
import CodeSamplesMultilingualDatasetGuideUpdateLocalizedAttributes1 from '/snippets/generated-code-samples/code_samples_multilingual_dataset_guide_update_localized_attributes_1.mdx';
...
- Suitable for languages that use spaces to separate words and share similar tokenization behavior (for example, English, French, Italian, Spanish, Portuguese).
...
- Use language-specific field names with a prefix or suffix (for example, title_fr, title_en, or fr_title).
...
- This lets you filter and search by language, even when multiple languages are stored in the same index.
...
- Meilisearch only chooses between the specified languages (for example, English vs German).
...
The [`localizedAttributes` setting](/reference/api/settings/get-localizedattributes) lets you explicitly define which languages are present in your dataset, and in which fields.
...
<CodeSamplesMultilingualDatasetGuideUpdateLocalizedAttributes1 />
