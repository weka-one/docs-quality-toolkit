...
Meilisearch lets you edit documents in place using [Rhai](https://rhai.rs/book/) scripting functions. Instead of fetching documents, modifying them externally, and re-indexing, you write a short function that Meilisearch applies to each matching document.
...
- **Computed fields**: derive new fields from existing ones (for example, concatenate `firstName` and `lastName` into `fullName`)
