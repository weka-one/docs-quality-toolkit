...
<Tip>
Filtering batches with [the `/batches` route](/reference/api/batches/list-batches) follows the same rules as filtering tasks. Many `/batches` parameters such as `uids` target the tasks included in batches, instead of the batches themselves.
</Tip>
