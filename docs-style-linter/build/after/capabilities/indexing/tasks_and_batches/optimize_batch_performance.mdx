...
import CodeSamplesUpdateFacetSearchSettings1 from '/snippets/generated-code-samples/code_samples_update_facet_search_settings_1.mdx';
...
| Description | Optimization |
|--------------|--------------|
| Builds structures for the [facet search API](/reference/api/facet-search/search-for-facet-values). | If you don't use the facet search API, [disable it](/reference/api/settings/update-facetsearch).|
...
| Description | Optimization |
|--------------|--------------|
| | Builds prefix data for autocomplete. Allows matching documents that begin with a specific query term, instead of only exact matches.| Disable [**prefix search**](/reference/api/settings/get-prefixsearch) (`prefixSearch: disabled`). _This can severely impact search result relevancy._ |
