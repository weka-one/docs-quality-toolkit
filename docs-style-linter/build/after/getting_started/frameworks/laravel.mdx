...
Laravel's official Docker development environment, Laravel Sail, comes with a Meilisearch service out-of-the-box. When running Meilisearch via Sail, Meilisearch's host is `http://meilisearch:7700` (instead of say, `http://localhost:7700`).
