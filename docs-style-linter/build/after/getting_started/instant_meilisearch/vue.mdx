...
The `ais-instant-search` widget is the mandatory wrapper that lets you configure your search. It takes two props: the `search-client` and the [`index-name`](/resources/internals/indexes#index-uid).
