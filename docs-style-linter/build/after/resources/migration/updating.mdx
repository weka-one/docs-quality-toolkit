...
If you're updating your Meilisearch instance on cloud platforms like DigitalOcean or AWS, ensure that you can connect to your cloud instance via SSH. Depending on the user you are connecting with (root, admin, and so on.), you may need to prefix some commands with `sudo`.
...
Click the project you want to update. Look for the "General settings" section at the top of the page.
...
This will open a pop-up with more information about the update process. Read it, then click "Update". The "Status" of your project will change from "running" to "updating".
...
Use the `taskUid` to [track the status](/reference/api/tasks/get-task) of your dump. The process can take some time to complete.
