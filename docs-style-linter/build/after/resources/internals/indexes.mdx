...
You can set the primary key for an index or let it be inferred by Meilisearch. Read more about [setting the primary key](/resources/internals/primary_key#setting-the-primary-key).

[Learn more about the primary field](/resources/internals/primary_key)
...
Filtering lets you refine your search based on different categories. For example, you could search for all movies of a certain `genre`: `Science Fiction`, with a `rating` above `8`.
...
Typo tolerance is a built-in feature that helps you find relevant results even when your search queries contain spelling mistakes or typos, for example, typing `chickne` instead of `chicken`. This setting lets you do the following for your index:
